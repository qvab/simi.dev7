...
function vd($data, $isPrint = false)
{
  if (!empty($isPrint)) {
    ?>
...
class core
{
  ...
  {
    $bd_host = "localhost";
    $bd_user = "u986993_vkapi";
    $bd_password = "1993mnxa";
    $bd_base = "u986993_vkapi";
    ...
    if (empty($this->errors)) {
      ...
      if (!empty($arParam["fields"]["values"])) {
        ...
        foreach ($arParam["fields"]["values"] as $idGroup => $arGroup) {
          ...
          if (count($arGroupValues[$idGroup]) > 1) {
              $arGroupString[$idGroup] = "(" . implode(",", $arGroupValues[$idGroup]) . ")";
          } else {
              $arGroupString[$idGroup] = "(" . $arGroupValues[$idGroup][0]. ")";
          }
        }
        $sSet = "(".implode(",", $arSet).")";
      ...
      if (!empty($rSqlError)) {
        ...
        $this->errors["core"]["__dbInsert"]["query"] = $sSql;
        return $this->errors;
      } else {
        return $rResponse;
      }
